...
uses
  {$IFNDEF CONSOLE}
    ...
    {$ENDIF}
  {$ENDIF}
  Data.DB, System.Generics.Collections, System.SysUtils, System.Classes;

type
  TSimpleEntity = class(TInterfacedPersistent)
  public
    {$IFNDEF CONSOLE}
    function Parse(const aForm: TForm): TSimpleEntity; overload; virtual;
    function Loads(const aForm: TForm; aEntity: TObject): TSimpleEntity; overload; virtual;
    {$ENDIF}
...
uses
  SimpleUtil, System.JSON, System.StrUtils;
...
function TSimpleEntity.Loads(const aForm: TForm; aEntity: TObject): TSimpleEntity;
begin
  Result := Self;
  TSimpleUtil.SetFormFromObject(aForm, aEntity);
end;
...
Initialization
    RegisterClass(TSimpleEntity);

finalization
  UnRegisterClass(TSimpleEntity);
