...
{$IFNDEF CONSOLE}
        ...
        function Update(aValues: String; aId: Integer): iSimpleDAO<T>; overload;
...
{$ENDIF}
        ...
        function Find(aWhere: string): T; overload;
...
begin
    try
        ...
        on E: Exception do
            TLog.New.error('Ocorreu um erro ao obter os dados provenientes do banco de dados.'+ #13 +
                'Detalhes: ' + E.Message)
...
begin
    try
        ...
        on E: Exception do
            TLog.New.error('Ocorreu erro ao deletar o registro!' + #13 + 'Detalhes: ' + E.Message)
    end;
end;
{$IFNDEF CONSOLE}
...
function TSimpleDAO<T>.Update(aValues: String; aId: Integer): iSimpleDAO<T>;
var
    aSQL: String;
    Entity: T;
begin
    try
        ...
        Entity := T.Create;
        try
            TSimpleSQL<T>.New(Entity).Update(aValues, aId, aSQL);
            FQuery.SQL.Clear;
            FQuery.SQL.Add(aSQL);
            TLog.New.debug(aSQL);
            Self.FillParameter(Entity, aId);
            FQuery.ExecSQL;
        finally
            FreeAndNil(Entity)
        end;
...
begin
    try
        ...
        on E: Exception do
            TLog.New.error('Ocorreu erro ao editar o registro!' + #13 + 'Detalhes: ' + E.Message)
    end;
end;
