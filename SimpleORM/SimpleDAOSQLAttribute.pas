...
Type
    TSimpleDAOSQLAttribute<T: class> = class(TInterfacedObject, iSimpleDAOSQLAttribute<T>)
    private
        [weak]
...
function TSimpleDAOSQLAttribute<T>.Limit(aSQL: String): iSimpleDAOSQLAttribute<T>;
begin
    Result := Self;
...
function TSimpleDAOSQLAttribute<T>.Join(aSQL: String): iSimpleDAOSQLAttribute<T>;
begin
    Result := Self;
    if Trim(aSQL) <> '' then
        FJoin := FJoin + ' ' + aSQL;
...
function TSimpleDAOSQLAttribute<T>.Fields(aSQL: String): iSimpleDAOSQLAttribute<T>;
begin
    Result := Self;
    if Trim(aSQL) <> '' then
